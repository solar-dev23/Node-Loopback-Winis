...
function initMap() {
  ...
  for(var i = 0; i < searchLocations.length; i++) {
    ...
    if (searchLocation.address) {
      addMarker(searchLocations[i].address, searchLocations[i].title + ' (Address)', fontawesome.markers.MAP_MARKER);
    }

    if (searchLocation.location) {
      addMarker(searchLocations[i].location, searchLocations[i].title + ' (GPS)', fontawesome.markers.LOCATION_ARROW);
    }
  }
